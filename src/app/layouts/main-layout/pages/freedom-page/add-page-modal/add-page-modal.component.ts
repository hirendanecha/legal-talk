...
export class AddFreedomPageComponent implements OnInit, AfterViewInit {
  @Input() title: string | undefined = 'Create Legal Topics';
        ...
        this.communityService.createCommunity(this.pageForm.value).subscribe(
          {
            next: (res: any) => {
              ...
              if (!res.error) {
                ...
                this.toastService.success('Law firm created successfully');
                // this.router.navigateByUrl('/home');
              }
            },
            error:
              (err) => {
                this.toastService.danger('Please change firm name. this firm name already in use.');
                this.spinner.hide();
              }
          });
        ...
        this.communityService.editCommunity(this.pageForm.value, this.data.Id).subscribe(
          {
            next: (res: any) => {
              ...
              if (!res.error) {
                this.submitted = true;
                // this.createCommunityAdmin(res.data);
                this.toastService.success('Your law firm edit successfully!');
                this.activeModal.close('success');
              }
            },
            error:
